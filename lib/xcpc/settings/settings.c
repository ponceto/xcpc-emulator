...
static const char arg_model[]        = "--model={value}";
...
static const char arg_manufacturer[] = "--manufacturer={value}";
...
static const char txt_model[]        = "cpc464, cpc664, cpc6128";
...
static const char txt_manufacturer[] = "Isp, Triumph, Saisho, Solavox, Awa, Schneider, Orion, Amstrad";
...
static const char txt_no_fps[]       = "don'tprint framerate";
...
static void print_usage(XcpcSettings* self)
{
    ...
    xcpc_println(format, arg_model       , txt_model       );
    ...
    xcpc_println(format, arg_manufacturer, txt_manufacturer);
...
XcpcSettings* xcpc_settings_construct(XcpcSettings* self)
{
    ...
    /* construct */ {
        ...
        self->state.model        = replace_setting(NULL, val_not_set, 0);
        ...
        self->state.manufacturer = replace_setting(NULL, val_not_set, 0);
...
XcpcSettings* xcpc_settings_destruct(XcpcSettings* self)
{
    ...
    /* destruct */ {
        ...
        self->state.model        = replace_setting(self->state.model       , NULL, 0);
        ...
        self->state.manufacturer = replace_setting(self->state.manufacturer, NULL, 0);
...
XcpcSettings* xcpc_settings_parse(XcpcSettings* self, int* argcp, char*** argvp)
{
    ...
    if((argvp != NULL) && (argvp != NULL) && (*argvp != NULL)) {
        ...
        while(argi < argc) {
            ...
            else if(check_arg(arg_model       , argument) != 0) { self->state.model        = replace_setting(self->state.model       , argument, 1); }
            ...
            else if(check_arg(arg_manufacturer, argument) != 0) { self->state.manufacturer = replace_setting(self->state.manufacturer, argument, 1); }
